...
int32_t CreateBuffer(FileBuffer* fileBuffer, uint8_t* buf, uint32_t size)
{
	...
	if (fileBuffer->lock < 0 )
	{
		printf("ERROR: sceKernelCreateLwMutex 0x%08x\n", ret);
		return ret;
	}
...
int32_t DeleteBuffer(FileBuffer* fileBuffer)
{
	...
	if ( !(fileBuffer->initFlag & RINGBUF_INIT_CREATE) )
	{
		ret = RINGBUF_ERROR_NOT_INITIALIZED;
		return ret;
	}
